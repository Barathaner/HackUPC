...
def get_meta_data():
    """
    Reads a CSV file, parses metadata from each link in the CSV rows, and returns the parsed data along with headers.

    Returns:
        tuple: A tuple containing:
               - A list of lists with parsed metadata for each link.
               - A list of headers describing each metadata field.
    """
    ...
    for item in data_from_csv:
        # Parse metadata for each non-empty link in item
        item_metadata = [parse_link_to_metadata(link) for link in item if link]
        parsed_metadata.append(item_metadata)

    print(parsed_metadata)

    # Define headers for the parsed data
    headers = ["year", "season", "product type", "section"]
